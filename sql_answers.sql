...
LEFT JOIN affiliates Tools ON Tools.type = 1 AND Tools.id = JobsTools.affiliate_id

LEFT JOIN jobs_career_paths JobsCareerPaths ON Jobs.id = JobsCareerPaths.job_id
LEFT JOIN affiliates CareerPaths ON CareerPaths.type = 3 AND CareerPaths.id = JobsCareerPaths.affiliate_id

LEFT JOIN jobs_rec_qualifications JobsRecQualifications ON Jobs.id = JobsRecQualifications.job_id
LEFT JOIN affiliates RecQualifications ON RecQualifications.type = 2 AND RecQualifications.id = JobsRecQualifications.affiliate_id
...
WHERE 
    ...
    AND EXISTS (
        SELECT 1 FROM jobs_practical_skills JobsPracticalSkills
        WHERE Jobs.id = JobsPracticalSkills.job_id
        AND (PracticalSkills.id = JobsPracticalSkills.practical_skill_id)
